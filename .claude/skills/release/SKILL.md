...
Prepare and publish a new release for Better Traders Guild.
...
!`bash -c 'TAG=$(git describe --tags --abbrev=0 2>/dev/null || echo "HEAD~10"); git log "$TAG..HEAD" --oneline --no-merges'`
...
- Rebuild (`dotnet build BetterTradersGuild.sln -c Release`) so the deployed
