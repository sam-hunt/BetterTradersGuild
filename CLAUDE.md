...
**Patches targeting other mods' methods:** must NOT ride the ctor-time `PatchAll` — applying a detour compiles the target, which runs its declaring type's static ctor before any defs exist, permanently breaking cctors that resolve defs inline (the v1.1.0 `CWTL_ChooseWhereToLand` incident). Gate the class's `Prepare` on `DeferredModPatches.PassActive` and add it to `DeferredModPatches.DeferredPatchClasses` (`Core/`), which applies them once per process from `BTGStartup.Run` after defs load.
