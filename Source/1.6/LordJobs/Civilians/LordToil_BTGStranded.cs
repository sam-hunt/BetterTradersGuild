...
namespace BetterTradersGuild.LordJobs.Civilians
{
    ...
    // reachability returns). The subroom door is open by now, so the family forages the wider
    // structure and wanders the wider nursery instead of staying boxed in. Assigns
    // BTG_StrandedAdult to the caretaker (still tends babies, then forages / calls a resupply
    // when starving) and BTG_StrandedChild to walking children (forage / eat / sleep / wander).
    //
