...
namespace BetterTradersGuild.RoomContents
{
    ...
    public class RoomContents_CrewQuarters : RoomContentsWorker
    {
        ...
        public override void FillRoom(Map map, LayoutRoom room, Faction faction, float? threatPoints)
        {
            ...
            Log.Message($"[Better Traders Guild] CrewQuarters: Packing {result.Subrooms.Count} subrooms into room at {roomRect}");

            // Log strip and region details for debugging
            foreach (var strip in result.Strips)
            {
                if (strip.Type == StripType.Subroom)
                {
                    string regionsInfo = string.Join(", ", strip.Regions.Select(r =>
                        $"x={r.MinX}-{r.MaxX} (width={r.Width}, excl={r.IsExclusionZone})"));
                    Log.Message($"[Better Traders Guild]   Strip z={strip.MinZ}-{strip.MaxZ} (depth={strip.Depth}, facing={strip.Facing}): Regions=[{regionsInfo}]");
                }
            }
            ...
            // 3. Call base to process XML-defined content (lockers spawn in corridors/exclusion zones)
            base.FillRoom(map, room, faction, threatPoints);

            // 4. Post-processing: spawn plants in any plant pots
            ThingDef fern = DefDatabase<ThingDef>.GetNamed("Plant_Fern", false);
            if (fern != null)
            {
                RoomPlantHelper.SpawnPlantsInPlantPots(map, roomRect, fern, growth: 1.0f);
            }
        }
        ...
        {
            Log.Message($"[Better Traders Guild] SpawnSubroomPrefab: {subroom.PrefabDefName} " +
                $"at MinX={subroom.MinX}, MinZ={subroom.MinZ}, Width={subroom.Width}, Depth={subroom.Depth}, " +
                $"Center=({subroom.CenterX},{subroom.CenterZ}), Rotation={subroom.Rotation}");
