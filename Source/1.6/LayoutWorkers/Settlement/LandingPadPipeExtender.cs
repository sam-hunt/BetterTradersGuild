...
namespace BetterTradersGuild.LayoutWorkers.Settlement
{
    ...
    ///
    /// LEARNING NOTE (Why Not Built-in Pathfinding?):
    /// RimWorld's PathFinder is pawn-centric (requires TraverseParms, factors doors/danger),
    /// FloodFiller is BFS-only and doesn't track parent cells for path reconstruction,
    /// and GenClosest/Reachability answer "find nearest" or "is reachable" questions.
    /// None support terrain-type-constrained pathing or full path collection, which we
    /// need here. Custom A* is the right fit for this niche use case.
    ///
