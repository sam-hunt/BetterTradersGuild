...
namespace BetterTradersGuild.Patches.Incidents
{
    ...
    // dense, defended stations, so raids there (e.g. the boosted Salvagers raids) should not get
    // the empty-space discount. Undoing the factor here preserves the behavior of the retired
    // PlanetTile.LayerDef getter patch, whose cloned layer def set raidPointsFactor=1.0.
    //
