...
namespace BetterTradersGuild.Tests.Tools
{
	...
	class RegenerateDiagrams
	{
		...
		static string RegenerateAllDiagrams(string content)
		{
			...
			var rotationPattern = new Regex(@"Assert\.Equal\(PlacementRotation\.(\w+), result\.Rotation\);");
			...
			foreach (Match match in matchList)
			{
				try
				{
					...
					PlacementCalculator.PlacementRotation expectedRotation = ParseRotationEnum(rotationMatch.Groups[1].Value);
					...
					// Extract wall segments from test assertions (instead of computing them)
					var requiredWalls = ExtractWallSegments(methodContent);
					Console.WriteLine($"  Found {requiredWalls.Count} wall assertions");
		...
		static int FindMethodStart(string content, int matchIndex)
		{
			...
			while (pos > 100)
			{
				...
				// Match both [Fact] and [Fact(Skip = "...")] by searching for "[Fact" prefix
		...
		/// <summary>
		/// Extracts wall segments from test assertions instead of computing them.
		/// Parses AssertWallSegment() helper method calls with named parameters.
		/// </summary>
		static List<PlacementCalculator.WallSegment> ExtractWallSegments(string methodContent)
		{
			var walls = new List<PlacementCalculator.WallSegment>();

			// Pattern to match: AssertWallSegment(result.RequiredWalls[0], startX: 2, startZ: 4, endX: 2, endZ: 9);
			var wallPattern = new Regex(
				@"AssertWallSegment\(result\.RequiredWalls\[\d+\],\s*" +
				@"startX:\s*(\d+),\s*" +
				@"startZ:\s*(\d+),\s*" +
				@"endX:\s*(\d+),\s*" +
				@"endZ:\s*(\d+)\);",
			...
			var matches = wallPattern.Matches(methodContent);

			foreach (Match match in matches)
			{
				int startX = int.Parse(match.Groups[1].Value);
				int startZ = int.Parse(match.Groups[2].Value);
				int endX = int.Parse(match.Groups[3].Value);
				int endZ = int.Parse(match.Groups[4].Value);

				walls.Add(new PlacementCalculator.WallSegment
				{
					StartX = startX,
					StartZ = startZ,
					EndX = endX,
		...
		/// <summary>
		/// Converts PlacementRotation enum name to PlacementRotation value.
		/// </summary>
		static PlacementCalculator.PlacementRotation ParseRotationEnum(string rotationName)
		{
			return rotationName switch
			{
				"North" => PlacementCalculator.PlacementRotation.North,
				"East" => PlacementCalculator.PlacementRotation.East,
				"South" => PlacementCalculator.PlacementRotation.South,
				"West" => PlacementCalculator.PlacementRotation.West,
				_ => throw new ArgumentException($"Invalid rotation name: {rotationName}")
			};
		}
	}
}
